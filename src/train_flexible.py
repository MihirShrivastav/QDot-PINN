...
from .potentials.biquadratic import BiquadraticParams, from_targets as biq_from_targets, v_biq_factory
from .potentials.gaussian import (
    GaussianParams, 
    from_physical_params as gauss_from_physical,
    from_dimensionless_targets as gauss_from_dimensionless,
    v_gaussian_factory,
    potential_info as gauss_info
)
...
# Type alias for potential parameters
PotentialParams = Union[BiquadraticParams, GaussianParams]

# ---- Helper functions for training loop ----

def vectorized_potential_v(p: PotentialParams, xy: torch.Tensor) -> torch.Tensor:
    """Vectorized potential evaluation for both biquadratic and Gaussian potentials."""
    if isinstance(p, BiquadraticParams):
        return vectorized_biq_v(p, xy)
    elif isinstance(p, GaussianParams):
        return vectorized_gauss_v(p, xy)
    else:
        raise ValueError(f"Unknown potential type: {type(p)}")
...
def vectorized_gauss_v(p: GaussianParams, xy: torch.Tensor) -> torch.Tensor:
    # xy: [N,2]
    x = xy[:, 0:1]
    y = xy[:, 1:2]
    ...
    # Gaussian wells (negative contribution = attractive)
    wells = p.v0 * (torch.exp(-r_left2 / (p.sigma ** 2)) + torch.exp(-r_right2 / (p.sigma ** 2)))
    
    # Central barrier (positive contribution = repulsive)
    barrier = p.v_b * torch.exp(-(x ** 2) / (p.sigma_b ** 2))
    
    # Total potential
    return p.v_off - wells + barrier + p.delta * x
...
def train_one_state(
    ...
    def vfun_batch(xy: torch.Tensor) -> torch.Tensor:
        return vectorized_potential_v(p, xy)
    ...
    # Optional LBFGS refinement (similar to original implementation)
    if lbfgs_iters and lbfgs_iters > 0:
        ...
        def closure():
            ...
            E2 = rayleigh_ritz_energy(model, vfun_batch, xy_q, w_q, energy_shift)
        ...
        opt2.step(closure)
        # Update best if improved after LBFGS
        with torch.no_grad():
            E2 = rayleigh_ritz_energy(model, vfun_batch, xy_q, w_q, energy_shift)
...
def parse_args() -> argparse.Namespace:
    ap = argparse.ArgumentParser(description="Train 1e PINN for DQD with flexible potential types")
    
    # Potential type selection
    ap.add_argument("--potential", type=str, choices=["biquadratic", "gaussian"], default="biquadratic",
                    help="Type of potential to use")
    
    # Common potential parameters
    ap.add_argument("--a", type=float, default=1.5, help="Half-separation between dots")
    ap.add_argument("--delta", type=float, default=0.0, help="Detuning parameter")
    
    # Biquadratic-specific parameters
    ap.add_argument("--c4", type=float, default=None, help="Quartic coefficient (biquadratic)")
    ap.add_argument("--c2y", type=float, default=None, help="Transverse harmonic coefficient (biquadratic)")
    ap.add_argument("--hbar-omega-x", type=float, default=3.0, help="meV; used if c4 not provided (biquadratic)")
    ap.add_argument("--hbar-omega-y", type=float, default=5.0, help="meV; used if c2y not provided (biquadratic)")
    
    # Gaussian-specific parameters
    ap.add_argument("--v0", type=float, default=10.0, help="Well depth (Gaussian, dimensionless)")
    ap.add_argument("--sigma", type=float, default=0.75, help="Well width (Gaussian, dimensionless)")
    ap.add_argument("--v-b", type=float, default=4.0, help="Central barrier height (Gaussian, dimensionless)")
    ap.add_argument("--sigma-b", type=float, default=0.3, help="Central barrier width (Gaussian, dimensionless)")
    ap.add_argument("--v-off", type=float, default=0.0, help="Baseline potential (Gaussian, dimensionless)")
    ...
    ap.add_argument("--lam-pde", type=float, default=2.5)
    ap.add_argument("--lam-norm", type=float, default=100.0)
    ap.add_argument("--lbfgs-iters", type=int, default=200, help="LBFGS refinement iterations (0 to disable)")
    
    # Symmetry constraints
    ap.add_argument("--lam-sym-odd", type=float, default=0.0, help="Weight for odd-parity penalty")
    ap.add_argument("--lam-sym-even", type=float, default=0.0, help="Weight for even-parity penalty")
    
    # Excited state / orthogonality options
    ap.add_argument("--state", type=int, default=0, help="0 = ground state, 1 = first excited")
    ap.add_argument("--ref-model", type=str, default=None, help="Path to reference model for orthogonality")
    ap.add_argument("--lam-ortho", type=float, default=10.0, help="Weight for orthogonality penalty")
    ...
    # Viz-only mode
    ap.add_argument("--viz-only", action="store_true", help="Only generate visualizations from saved model")
    ap.add_argument("--model", type=str, default=None, help="Path to model weights for viz-only mode")
...
def create_potential_params(args, mat: GaAs) -> PotentialParams:
    """Create potential parameters based on command line arguments."""
    if args.potential == "biquadratic":
        if args.c4 is None or args.c2y is None:
            return biq_from_targets(
                a=args.a, 
                hbar_omega_x_meV=args.hbar_omega_x, 
                hbar_omega_y_meV=args.hbar_omega_y, 
                delta=args.delta, 
                mat=mat
            )
        else:
            return BiquadraticParams(a=args.a, c4=float(args.c4), c2y=float(args.c2y), delta=args.delta)
    
    elif args.potential == "gaussian":
        return gauss_from_dimensionless(
            a=args.a,
            well_depth=args.v0,
            well_width=args.sigma,
            barrier_height=args.v_b,
            barrier_width=args.sigma_b,
            delta=args.delta,
            v_off=getattr(args, 'v_off')
        )
    
    else:
        raise ValueError(f"Unknown potential type: {args.potential}")

def create_potential_factory(p: PotentialParams) -> Callable[[float, float], float]:
    """Create potential function factory based on parameter type."""
    if isinstance(p, BiquadraticParams):
        return v_biq_factory(p)
    elif isinstance(p, GaussianParams):
        return v_gaussian_factory(p)
    else:
        raise ValueError(f"Unknown potential type: {type(p)}")

def run_viz_only_mode(args, mat: GaAs, outdir: Path):
    """Run visualization-only mode: load trained model and generate all outputs."""
    print("Running in visualization-only mode...")
    
    # Try to load configuration from existing run
    cfg = None
    ...
    # Default parameters
    p = create_potential_params(args, mat)
    ...
    if cfg_path.exists():
        try:
            ...
            potential_type = cfg.get("potential_type", args.potential)
            params_dict = cfg.get("params", {})
            
            if potential_type == "biquadratic" and all(k in params_dict for k in ("a", "c4", "c2y", "delta")):
                p = BiquadraticParams(**params_dict)
            elif potential_type == "gaussian" and all(k in params_dict for k in ("a", "v0", "sigma")):
                p = GaussianParams(**params_dict)
            
            # Load box and architecture parameters
            box = cfg.get("box", {})
            ...
            hidden = int(args_cfg.get("hidden", hidden))
            layers = int(args_cfg.get("layers", layers))
            
            print(f"Loaded configuration from {cfg_path}")
            
        except Exception as e:
            print(f"Warning: Could not load config ({e}), using command-line parameters")
    
    # Print potential info
    if isinstance(p, BiquadraticParams):
        print(f"Using biquadratic params: a={p.a:.3f}, c4={p.c4:.3f}, c2y={p.c2y:.3f}, delta={p.delta:.3f}")
    elif isinstance(p, GaussianParams):
        print(f"Using Gaussian params: a={p.a:.3f}, v0={p.v0:.3f}, sigma={p.sigma:.3f}")
        print(f"  Barrier: v_b={p.v_b:.3f}, sigma_b={p.sigma_b:.3f}, v_off={p.v_off:.3f}, delta={p.delta:.3f}")
        try:
            from .potentials.gaussian import potential_info as gauss_info
            print(gauss_info(p, mat))
        except ImportError:
            pass
    ...
    vfun_batch = lambda xy: vectorized_potential_v(p, xy)
    energy_shift = calculate_energy_shift(p)
    E_fin = rayleigh_ritz_energy(model, vfun_batch, xy_q, w_q, energy_shift)
    ...
    # Create potential function for plotting
    vfun = create_potential_factory(p)
    ...
    print(f"Final energy: {E_fin.item():.6f}, residual: {Lres_fin.item():.3e}")
    return
...
def main():
    ...
    # Viz-only mode: load existing model and regenerate outputs
    outdir = Path(args.outdir)
    if args.viz_only:
        return run_viz_only_mode(args, mat, outdir)

    # Create potential parameters
    p = create_potential_params(args, mat)
